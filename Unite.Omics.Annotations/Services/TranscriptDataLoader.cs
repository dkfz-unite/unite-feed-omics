...
public class TranscriptDataLoader
{
    private readonly IDbContextFactory<DomainDbContext> _dbContextFactory;
    private readonly EnsemblApiClient1 _ensemblApiClient;


    public TranscriptDataLoader(IEnsemblDataOptions ensemblOptions, IDbContextFactory<DomainDbContext> dbContextFactory)
    {
    ...
    public async Task<TranscriptModel[]> LoadById(string[] keys)
    {
        ...
        var newResources = await _ensemblApiClient.FindById<TranscriptResource>(newKeys, length: true, expand: false);

        return Enumerable.Concat(existingEntities.Select(Convert), newResources.Select(Convert)).ToArray();
    }

    public async Task<TranscriptModel[]> LoadBySymbol(string[] keys)
    {
        ...
        var newResources = await _ensemblApiClient.FindByName<TranscriptResource>(newKeys, length: true, expand: false);

        return Enumerable.Concat(existingEntities.Select(Convert), newResources.Select(Convert)).ToArray();
    }


    private Transcript[] LoadEntities(Expression<Func<Transcript, bool>> predicate)
    {
        using var dbContext = _dbContextFactory.CreateDbContext();

        return dbContext.Set<Transcript>()
            .AsNoTracking()
    ...
    private static TranscriptModel Convert(Transcript entity)
    {
    ...
    private static TranscriptModel Convert(TranscriptResource resource)
    {
        return new TranscriptModel
        {
            Id = null,
            ...
            Gene = new () { StableId = resource.GeneId }
        };
    }
}
